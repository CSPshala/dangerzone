...
//
//	Author			:	David Brown (DB) - Modified by JC Ricks for wide char conformity
//	Based in part on:
...
#ifndef DIERRBOX
	#define DIERRBOX(hWnd, errorText)	{ MessageBox(hWnd, errorText, L"CSGD_DirectInput Error", MB_OK | MB_ICONEXCLAMATION); }
#endif
...
bool CSGD_DirectInput::InitKeyboard(HWND hWnd, bool bIsExclusive)
{
	...
	if (m_pKeyboard)
	{
		DIERRBOX(hWnd, L"Keyboard has already been initialized")
		return false;
	}
...
bool CSGD_DirectInput::InitMouse(HWND hWnd, bool bIsExclusive)
{
	...
	if (m_pMouse)
	{
		DIERRBOX(hWnd, L"Mouse has already been initialized")
		return false;
	}
...
bool CSGD_DirectInput::InitJoysticks(HWND hWnd, bool bIsExclusive)
{
	...
	if (m_vpJoysticks.size() > 0)
	{
		DIERRBOX(hWnd, L"Joysticks have already been initialized")
		return false;
	}
...
CSGD_DIKeyboard::CSGD_DIKeyboard(LPDIRECTINPUT8 pDI, HWND hWnd, bool bIsExclusive)
{
	...
	{
		DIERRBOX(hWnd, L"Failed to create Keyboard device.")
	}

	//	Set the Data Format for the Keyboard.
	if (FAILED( m_lpDevice->SetDataFormat(&c_dfDIKeyboard) ))
	{
		DIERRBOX(hWnd, L"Failed to set data format on Keyboard.")
	}
	...
	{
		DIERRBOX(hWnd, L"Failed to set cooperative level on Keyboard.")
	}
	...
	if (FAILED( m_lpDevice->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph) ))
		DIERRBOX(hWnd, L"Could not Set the Properties for Buffered Input for Keyboard.");

	//	Acquire the Keyboard.
	if (FAILED( m_lpDevice->Acquire() ))
	{
		DIERRBOX(hWnd, L"Failed to acquire Keyboard.")
	}	
...
CSGD_DIMouse::CSGD_DIMouse(LPDIRECTINPUT8 pDI, HWND hWnd, bool bIsExclusive)
{
	...
	{
		DIERRBOX(hWnd, L"Failed to create Mouse device.")
	}

	//	Set the Data Format for the Mouse.
	if (FAILED( m_lpDevice->SetDataFormat(&c_dfDIMouse2) ))
	{
		DIERRBOX(hWnd, L"Failed to set data format on Mouse.")
	}
	...
	if (FAILED( m_lpDevice->SetCooperativeLevel(hWnd, dwFlags) ))
	{
		DIERRBOX(hWnd, L"Failed to set cooperative level on Mouse.")
	}
	...
	//	Set the buffer size on the device.
	if (FAILED( m_lpDevice->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph) ))
		DIERRBOX(hWnd, L"Could not Set the Properties for Buffered Input for Mouse.");

	//	Acquire the Mouse.
	if (FAILED( m_lpDevice->Acquire() ))
	{
		DIERRBOX(hWnd, L"Failed to acquire Mouse.")
	}	
...
{
	//	For error reporting
	wchar_t szErrorBuffer[256];

	//	Get the Joystick Name and store it.
	//strcpy_s(m_szJoyName, _countof(m_szJoyName), (char*)lpdidi->tszProductName);
	wcscpy_s(m_szJoyName, _countof(m_szJoyName),lpdidi->tszProductName);


	// Remember if it is an Xbox 360 pad
	m_bIsXbox360Pad = (wcscmp(m_szJoyName, L"Xbox 360 Wireless Receiver for Windows") == 0 || wcscmp(m_szJoyName, L"Controller (XBOX 360 For Windows)") == 0) ? true : false;

	//	Create the Joystick Device.
	if (FAILED( pDI->CreateDevice(lpdidi->guidInstance, &m_lpDevice, NULL) ))
	{
		swprintf_s(szErrorBuffer, _countof(szErrorBuffer), L"Failed to create Joystick device: %s", m_szJoyName);
	...
	{
		swprintf_s(szErrorBuffer, _countof(szErrorBuffer), L"Failed to set data format on Joystick: %s", m_szJoyName);
		DIERRBOX(hWnd, szErrorBuffer)
	}
	...
	if (FAILED( m_lpDevice->SetCooperativeLevel(hWnd, dwFlags) ))
	{
		swprintf_s(szErrorBuffer, _countof(szErrorBuffer), L"Failed to set cooperative level on Joystick: %s", m_szJoyName);
		DIERRBOX(hWnd, szErrorBuffer)
	}
	...
	if (FAILED( m_lpDevice->SetProperty(DIPROP_BUFFERSIZE, &dipdw.diph) ))
	{
		swprintf_s(szErrorBuffer, _countof(szErrorBuffer), L"Could not Set the Properties for Buffered Input for Joystick: %s", m_szJoyName);
		DIERRBOX(hWnd, szErrorBuffer)
	}
	...
	if (FAILED( m_lpDevice->Acquire() ))
	{
		swprintf_s(szErrorBuffer, _countof(szErrorBuffer), L"Failed to acquire Joystick: %s", m_szJoyName);
		DIERRBOX(hWnd, szErrorBuffer)
	}
